...
)
{
	...
	// Driver must be loaded in PASSIVE LEVEL in order to perform Read and Write operations
	if (KeGetCurrentIrql() != PASSIVE_LEVEL)
		return STATUS_INVALID_DEVICE_STATE;

	// Open file and set pointers
	#define  BUFFER_SIZE 0x200
	CHAR     buffer[BUFFER_SIZE] = "MEOK";
	...
	// Write to NTFS drive and read changed contents
	ntstatus3 = ZwWriteFile(handle, NULL, NULL, NULL, &ioStatusBlock,
		buffer, BUFFER_SIZE, &frAddr, NULL);
	...
	// ---------------------------------------------------------------------------
	// Print ntstatus to file (need interaction with Kernel mode).
...
#define  BUFFER_SIZE2 0x200
	...
	sprintf(buffer2, "hello%d", ntstatus3);

	ntstatus = ZwWriteFile(handle2, NULL, NULL, NULL, &ioStatusBlock2,
		buffer2, BUFFER_SIZE2, NULL, NULL);
	ZwClose(handle2);
